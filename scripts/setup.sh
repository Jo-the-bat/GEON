...
echo ""
echo -e "${CYAN}============================================${NC}"
echo -e "${CYAN}   HEGO — Setup Script${NC}"
echo -e "${CYAN}============================================${NC}"
echo ""
...
if ! command -v docker &>/dev/null; then
    ...
    if docker info 2>/dev/null | grep -q "rootless"; then
        ...
        echo "      HEGO is designed for rootless Docker. See: https://docs.docker.com/engine/security/rootless/"
    fi
fi
...
if [ -n "$TOTAL_MEM_KB" ]; then
    ...
    if [ "$TOTAL_MEM_GB" -ge 8 ]; then
        ...
        warn "Available RAM: ${TOTAL_MEM_GB} GB. HEGO recommends at least 8 GB (16 GB ideal)."
    fi
fi
