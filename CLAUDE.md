...
**Contexte** : Projet personnel de GEON, professionnel en cybersécurité (analyste SOC, administration Linux/Docker), destiné à servir de portfolio pour une candidature en Master Relations Internationales. Le projet doit démontrer la capacité à croiser analyse technique et compréhension géopolitique.
...
- Le domaine cible est `hego.example.com` avec un certificat Let's Encrypt
...
Fenêtre: 12 jours
Dashboard: https://hego.example.com/kibana/app/dashboards#/view/correlations
```
...
```bash
# === DOMAINE ===
HEGO_DOMAIN=hego.example.com
HEGO_EMAIL=contact@example.com
...
ALERT_EMAIL_FROM=hego@example.com
...
3. TLS via Certbot pour hego.example.com
...
curl -sk https://hego.example.com/kibana/api/status | jq .status
