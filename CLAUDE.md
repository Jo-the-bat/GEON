...
**Metriques** : chaque job est instrumente (Prometheus, port 9108 du conteneur, scrape par geon-prometheus) — `geon_job_runs_total{job,status}`, `geon_job_duration_seconds`, `geon_job_last_success_timestamp_seconds`, `geon_job_last_result_docs`. Une regle d'alerte Grafana provisionnee (`docker/grafana/provisioning/alerting/geon-alerting.yml`) previent sur Discord si l'ingestion GDELT (1h) ou le moteur de correlation (2h) s'arretent, ou si un job echoue — UNE seule regle combinee (instances par cause via labels) car le provisioning de Grafana 10.4.15 rejette toute deuxieme regle avec un faux conflit de titre (bug verifie empiriquement ; revoir apres upgrade Grafana >= 11). Si `DISCORD_WEBHOOK_URL` est vide dans le `.env`, une URL sentinelle evite le crash de Grafana — les notifications partent des que la variable est remplie + grafana recree.
